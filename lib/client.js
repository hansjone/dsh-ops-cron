window.__ModuleLoader__.load({
  id: '@dsh-external/dsh-cron-tasks',
  factory: (require) => {
    ...
    const name = 'dsh-cron-tasks'
    const NS = 'dsh-cron-tasks'
...
`
    ...
    const zh = {
      ...
      hint: '任务在左侧「新会话」下方进入。运行结果会出现在历史行里，点击记录会打开和普通会话一样的对话页，可以继续聊。',
      ...
      editorLead: '保存后按日程在新会话里执行。点击下方某次运行，右侧会打开那次对话，可以继续。',
    ...
    const en = {
      ...
      hint: 'Open from under New Session. Run output appears in history; click a run to continue in the normal chat view.',
      ...
      editorLead: 'Runs start a new session. Click a run below to open that conversation and continue it.',
            ...
            return h('div', { className: skin.group, key: job.id },
              h('div', {
                ...
                h('span', { className: skin.projectText },
                  h('span', { className: skin.title }, paused ? `${job.name} · ${t('paused')}` : job.name)),
    ...
    function JobEditor({ t, form, setForm, isNew, error, lastOutput, nextRunAt, lastRunAt, workspaces, onSave, onRun, onPause, onRemove }) {
    ...
    function CronApp({ t, faces }) {
      ...
      const load = useCallback(async () => {
        try {
          const [jobBody, histBody, wsBody] = await Promise.all([
            api('/jobs'),
            api('/history'),
      ...
      useEffect(() => {
        ...
        setForm((prev) => {
          ...
          return cwd ? { ...prev, cwd } : prev
        })
      }, [cronMode, selection.type])
    ...
    function isCronTitle(text) {
      ...
      return value.startsWith('定时任务') || value.startsWith(TITLE_PREFIX.trim()) || value.includes(TITLE_PREFIX)
    }
    ...
    function syncEntryLook(entry, found) {
      ...
      entry.style.height = cs.height
      entry.style.padding = cs.padding
      entry.style.gap = cs.gap
        ...
        try {
          if (!entry || !entry.isConnected) {
            ...
            entry.addEventListener('click', (event) => {
              ...
              cronOn = !cronOn
              setCronMode(findSidebarRoot(found), cronOn, entry, t)
            })
        ...
        async openSession(sessionId) {
          ...
          while (Date.now() < deadline) {
            ...
            try {
              ...
              lastError = error
              console.warn('[dsh-cron-tasks] open session failed', sessionId, error)
              try { await sessions.refresh?.() } catch { /* ignore */ }
              await sleep(80)
              continue
            }
          ...
          if (lastError) console.warn('[dsh-cron-tasks] giving up open', sessionId, lastError)
          else console.warn('[dsh-cron-tasks] open session timed out', sessionId)
          return false
        },
        clearSession() {
          try {
      ...
      ctx.effect(() => {
        ...
        const consider = async (sessions) => {
          ...
          try {
            ...
            console.warn('[dsh-cron-tasks] fork rename failed', row.id, error)
          }
          try {
            await api(`/sessions/${encodeURIComponent(row.id)}/adopt`, { method: 'POST' })
          } catch (error) {
            console.warn('[dsh-cron-tasks] fork adopt failed', row.id, error)
          }
