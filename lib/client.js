...
`
              ...
              h('div', {
                ...
                h('span', { className: skin.projectText },
                  h('span', { className: skin.title }, paused
                    ? `${job.name} · ${t('paused')}`
                    : (jobModelLabel(job) ? `${job.name} · ${jobModelLabel(job)}` : job.name))),
    ...
    function JobEditor({ t, form, setForm, isNew, error, lastOutput, nextRunAt, lastRunAt, workspaces, catalog, onSave, onRun, onPause, onRemove }) {
    ...
    function CronApp({ t, faces }) {
      ...
      }, [cronMode, selection.type, catalog.current])
