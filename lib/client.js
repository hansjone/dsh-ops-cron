...
`
    ...
    const zh = {
      ...
      emptyJobs: '还没有运维定时。点右上角 + 新建。', next: '下次', last: '上次',
      ...
      modelHint: '运维定时会消耗这个模型的额度。指定后不会跟着聊天模型变。',
      editorLead: '保存后按日程在新会话里执行；若投递为 IM，结束后还会发摘要到 WhatsApp/IM。点击下方某次运行，右侧会打开那次对话，可以继续。',
      ...
      deliveryHint: 'Web 默认 DSH；WhatsApp 会话里由 Agent 创建时可自动匹配投递目标。也可粘贴 IM「投递设置」里的 botId/targetId。',
    }
    const en = {
      title: 'Ops schedule',
      ...
      entry: 'Ops schedule', entryLabel: 'Open ops schedule',
      ...
      editorLead: 'Runs start a new session. IM delivery also sends a summary via dshIm after the turn. Click a run below to open that conversation.',
      ...
      imBotId: 'IM Bot ID',
      imTargetId: 'IM Target ID',
      deliveryHint: 'Web defaults to DSH. WhatsApp sessions can auto-match a delivery target. Or paste botId/targetId from IM delivery settings.',
    }
    ...
    function isCronTitle(text) {
      ...
      return value.startsWith('运维定时') || value.startsWith(TITLE_PREFIX.trim()) || value.includes(TITLE_PREFIX)
    }
    ...
    function shouldPromoteCronFork(row, cronSessionIds, previousId) {
      ...
      const looksCron = title.startsWith(TITLE_PREFIX) || title.startsWith('运维定时')
