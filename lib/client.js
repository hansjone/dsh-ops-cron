window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const TITLE_PREFIX_EN = 'Scheduled tasks · '
    /** Live translator from ctx.locale.bind; falls back until apply() wires it. */
    let translate = (key) => en[key] || zh[key] || key
    function uiT(key) {
      try { return translate(key) } catch { return en[key] || zh[key] || key }
    }
    function resolveUiLang(ctx) {
        ...
        try {
          ...
          if (typeof bound === 'function') {
            return (key, params) => {
              ...
              const dict = resolveUiLang(ctx) === 'zh' ? zh : en
...
.dsh-ct-dirChildren{padding-left:16px;box-sizing:border-box}
...
.dsh-ct-check{flex-direction:row!important;align-items:center;gap:8px!important}
.dsh-ct-check input{width:auto;margin:0}
.dsh-ct-state{display:inline-flex;align-items:center;gap:4px;font-size:11px;font-weight:600;letter-spacing:.02em;padding:1px 6px;border-radius:999px;border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);vertical-align:middle;margin-left:6px}
.dsh-ct-state[data-state=running],.dsh-ct-state[data-state=pending]{color:var(--dsw-alias-state-business-primary);border-color:currentColor}
.dsh-ct-state[data-state=succeeded]{color:var(--dsw-alias-state-success-primary, #1a7f37);border-color:currentColor}
.dsh-ct-state[data-state=failed],.dsh-ct-state[data-stuck=true]{color:var(--dsw-alias-state-error-primary);border-color:currentColor}
.dsh-ct-state[data-state=paused]{opacity:.75}
.dsh-ct-labels{color:var(--dsw-alias-label-tertiary);font-size:11px;font-weight:400;margin-left:6px}
...
`
    ...
    const zh = {
      ...
      hint: '每次执行会新开会话跑任务。默认不会把摘要注入创建时的有效会话；需要跟进时可在任务里勾选「镜像回原会话」。完整工具轨迹仍可从历史打开。',
      ...
      persistHistory: '历史保留',
      persistRetain: '保留最近 N 条（跟随全局）',
      persistForever: '永久保留',
      persistArchive: '归档到远程 endpoint',
      archiveEndpoint: '归档 URL',
      archiveEndpointHint: 'persist_history=archive 时 POST 终态运行到该 http(s) 地址。',
      watch: '监听绑定 (JSON)',
      watchHint: '可选。例 {"taskId":"..."} 或 {"labels":{"task":"theory"},"match":"all"}',
      progressFile: '进度文件',
      progressFileHint: '相对工作目录或绝对路径；任务运行时写入 JSON 快照。',
      ...
      editorLead: '到点仍会新开会话执行。摘要默认只留在运行历史；勾选「镜像回原会话」才会写入创建时的有效会话。IM 投递仍会经 WhatsApp/IM 发出。完整工具轨迹可从下方运行记录打开。',
    ...
    const en = {
      ...
      hint: 'Each fire runs in a fresh session. By default the summary is not injected into the origin WhatsApp/Web chat; enable “Mirror into origin session” on a job when you want follow-up context there. Full tool traces stay in run history.',
      ...
      labelsPlaceholder: 'role=worker\ntask=theory',
      persistHistory: 'History retention',
      persistRetain: 'Retain last N (global setting)',
      persistForever: 'Keep forever',
      persistArchive: 'Archive to remote endpoint',
      archiveEndpoint: 'Archive URL',
      archiveEndpointHint: 'When archive is selected, terminal runs are POSTed to this http(s) URL.',
      watch: 'Watch binding (JSON)',
      watchHint: 'Optional. e.g. {"taskId":"..."} or {"labels":{"task":"theory"},"match":"all"}',
      progressFile: 'Progress file',
      progressFileHint: 'Path relative to cwd or absolute; the job writes a JSON snapshot while running.',
      ...
      editorLead: 'Fires still run in a new session. Summaries stay in run history by default; enable “Mirror into origin session” to append them to the origin chat. IM delivery still sends via WhatsApp/IM. Open a run below for the full tool trace.',
    ...
    function SettingsCard(props) {
      const t = typeof props.t === 'function'
        ? props.t
        : (key) => (resolveUiLang() === 'zh' ? zh : en)[key] || en[key] || zh[key] || key
    ...
    async function api(path, options = {}) {
      if (!hasUdsLoginCookie() && path !== '/health') {
        throw new Error(uiT('loginRequired'))
      }
    ...
    function jobToForm(job) {
      return {
        ...
        watchText: job.watch ? JSON.stringify(job.watch, null, 2) : '',
        progressFile: job.progress?.channel?.file || '',
      }
    }
    ...
    function jobStateKey(job) {
      if (job?.enabled === false) return 'paused'
      return String(job?.state || 'idle')
    }

    function jobStateLabel(job, t) {
      const key = jobStateKey(job)
      const map = {
        idle: t('stateIdle'),
        pending: t('statePending'),
        running: t('stateRunning'),
        succeeded: t('stateSucceeded'),
        failed: t('stateFailed'),
        paused: t('statePaused'),
      }
      const base = map[key] || key
      return job?.stuck ? `${base} · ${t('stateStuck')}` : base
    }

    function formatLabelsText(labels) {
    ...
    function persistHistoryKind(job) {
      const kind = job?.persistHistory?.kind
      if (kind === 'forever' || kind === 'archive') return kind
      return 'retain'
    }

    function labelsInline(job) {
    ...
    function JobList({ t, jobs, runs, selection, expanded, viewer, error, onSelectJob, onSelectRun, onNew, onRun, onToggle, onRemove, onToggleGroup }) {
      ...
      function renderRunRows(job) {
        ...
        return jobRuns.map((run) => {
          ...
          return h('div', {
            ...
            h('span', { className: skin.title },
              (run.state || run.status || '').toString(),
              ' · ',
              (run.summary || '').split('\n')[0] || '',
            ),
      ...
      // Directory node: job folder → session leaves. When nestedUnderUser, this is level 2.
      function renderJobGroup(job, nestedUnderUser = false) {
        ...
        return h('div', { className: skin.group, key: job.id },
          h('div', {
            ...
            role: 'treeitem',
            'aria-expanded': open,
            'aria-level': nestedUnderUser ? 2 : 1,
          ...
          h('div', { className: 'dsh-ct-runs', 'data-open': open ? 'true' : 'false' },
            h('div', { className: joinClass('dsh-ct-runsInner', 'dsh-ct-dirChildren') },
              renderRunRows(job),
            ),
          ),
        )
      ...
      // Super / fallback_admin: user folder → job folder → session leaf.
      function renderGrouped() {
        ...
        const ordered = [...groups.values()].sort((a, b) => {
          ...
          return String(a.label).localeCompare(String(b.label), resolveUiLang() === 'zh' ? 'zh' : 'en')
        })
        return ordered.map((group) => {
          const folderId = `owner:${group.key}`
          const open = expanded[folderId] === true || !!needle
          ...
          return h('div', { className: skin.group, key: folderId },
            ...
            h('div', { className: 'dsh-ct-runs', 'data-open': open ? 'true' : 'false' },
              h('div', { className: joinClass('dsh-ct-runsInner', 'dsh-ct-dirChildren') },
                group.jobs.map((job) => renderJobGroup(job, true)),
              ),
            ),
          )
        })
    ...
    /** Prefer open session cwd for elevated viewers; else provisioned path. */
    function preferredNewJobCwd(viewer, sessionCwd) {
      ...
      if ((role === 'super_admin' || role === 'fallback_admin') && session) return session
    ...
    function JobEditor({ t, form, setForm, isNew, error, lastOutput, nextRunAt, lastRunAt, jobMeta, workspaces, catalog, presets, imCatalog, viewer, onSave, onRun, onPause, onRemove }) {
      return h('div', { className: 'dsh-ct-editor' },
        ...
        !isNew && jobMeta
          ? h('p', { className: 'dsh-ct-editorLead' },
            h('span', {
              className: 'dsh-ct-state',
              'data-state': jobStateKey(jobMeta),
              'data-stuck': jobMeta.stuck ? 'true' : 'false',
            }, `${t('state')}: ${jobStateLabel(jobMeta, t)}`),
            jobMeta.watch ? ` · watch` : '',
            jobMeta.progress?.channel?.file ? ` · progress` : '',
          )
          : null,
        ...
        h('label', null, t('labels'),
          h('textarea', {
            value: form.labelsText || '',
            placeholder: t('labelsPlaceholder'),
            onChange: (e) => setForm({ ...form, labelsText: e.target.value }),
            rows: 3,
          }),
        ),
        h('span', { className: 'dsh-ct-cwdHint' }, t('labelsHint')),
        h('label', null, t('persistHistory'),
          h('select', {
            value: form.persistKind || 'retain',
            onChange: (e) => setForm({ ...form, persistKind: e.target.value }),
          },
            h('option', { value: 'retain' }, t('persistRetain')),
            h('option', { value: 'forever' }, t('persistForever')),
            h('option', { value: 'archive' }, t('persistArchive')),
          ),
        ),
        form.persistKind === 'archive'
          ? h(React.Fragment, null,
            h('label', null, t('archiveEndpoint'), h('input', {
              value: form.archiveEndpoint || '',
              placeholder: 'https://example.com/cron-archive',
              onChange: (e) => setForm({ ...form, archiveEndpoint: e.target.value }),
            })),
            h('span', { className: 'dsh-ct-cwdHint' }, t('archiveEndpointHint')),
          )
          : null,
        h('label', null, t('watch'),
          h('textarea', {
            value: form.watchText || '',
            placeholder: '{"labels":{"task":"theory"},"match":"all"}',
            onChange: (e) => setForm({ ...form, watchText: e.target.value }),
            rows: 3,
          }),
        ),
        h('span', { className: 'dsh-ct-cwdHint' }, t('watchHint')),
        h('label', null, t('progressFile'),
          h('input', {
            value: form.progressFile || '',
            placeholder: 'progress.json',
            onChange: (e) => setForm({ ...form, progressFile: e.target.value }),
          }),
        ),
        h('span', { className: 'dsh-ct-cwdHint' }, t('progressFileHint')),
        ...
        h('label', { className: 'dsh-ct-check' },
          h('input', {
            type: 'checkbox',
            checked: form.mirrorToSession === true,
            onChange: (e) => setForm({ ...form, mirrorToSession: e.target.checked }),
          }),
          t('mirrorToSession'),
        ),
        h('span', { className: 'dsh-ct-cwdHint' }, t('mirrorToSessionHint')),
    ...
    function CronApp({ t, faces }) {
      ...
      useEffect(() => {
        ...
        const offs = []
        window.addEventListener('dsh-ct-locale', bump)
        offs.push(() => window.removeEventListener('dsh-ct-locale', bump))
      ...
      useEffect(() => {
        ...
        // Keep the directory tree collapsed on first open; expand only on explicit click.
      }, [cronMode, jobs])
      ...
      function expandJobPath(jobId) {
        ...
        const owner = `owner:${String(job?.ownerEmpNo || '__unassigned__')}`
        setExpanded((prev) => ({ ...prev, [jobId]: true, [owner]: true }))
      }
      ...
      async function selectRun(jobId, run) {
        ...
        if (opened === false) setError(uiT('runOpenFailed'))
      }
        ...
        try {
          ...
          const payload = {
            ...
            persistHistory: form.persistKind === 'forever'
              ? { kind: 'forever' }
              : form.persistKind === 'archive'
                ? { kind: 'archive', endpoint: String(form.archiveEndpoint || '').trim() }
                : { kind: 'retain' },
          }
          const watchRaw = String(form.watchText || '').trim()
          if (watchRaw) {
            try {
              payload.watch = JSON.parse(watchRaw)
            } catch {
              throw new Error('watch must be valid JSON')
            }
          } else {
            payload.watch = null
          }
          const progressFile = String(form.progressFile || '').trim()
          if (progressFile) {
            payload.progress = { channel: { file: progressFile }, metrics: [] }
          } else {
    ...
    function isCronTitle(text) {
      ...
      return value.startsWith('定时任务')
        ...
        || value.startsWith(TITLE_PREFIX_EN.trim())
        || value.includes(TITLE_PREFIX_EN)
        || /^scheduled tasks/i.test(value)
    }
    ...
    function workspaceTitleFromCronFork(title) {
      ...
      if (value.startsWith(TITLE_PREFIX_EN)) return value.slice(TITLE_PREFIX_EN.length).trim() || value
      ...
      if (/^scheduled tasks(?:\s*·\s*|\s+)/i.test(value)) {
        const stripped = value.replace(/^scheduled tasks(?:\s*·\s*|\s+)/i, '').trim()
        return stripped || value
      }
      return value
    }
    ...
    function shouldPromoteCronFork(row, cronSessionIds, previousId) {
      ...
      const looksCron = title.startsWith(TITLE_PREFIX)
        || title.startsWith(TITLE_PREFIX_EN)
        || title.startsWith('定时任务')
        || title.startsWith('运维定时')
        || /^scheduled tasks/i.test(title)
    ...
    function installChrome(t, faces, ctx) {
      let entry = null
      let listRoot = null
      let appRoot = null
      let cronOn = false
      ...
      function onLocaleChange() {
        syncEntryCopy()
        window.dispatchEvent(new CustomEvent('dsh-ct-locale'))
        if (appRoot) appRoot.render(h(CronApp, { t, faces }))
      }
      ...
      try {
        ...
        if (typeof off === 'function') localeOffs.push(off)
      } catch { /* optional */ }
      try {
        const off = ctx?.locale?.subscribe?.(onLocaleChange)
        if (typeof off === 'function') localeOffs.push(off)
      } catch { /* optional */ }
      // document.lang flips with DSH syncDocumentLanguage — catch hosts without locale events.
      let lastLang = typeof document !== 'undefined' ? document.documentElement?.lang : ''
      ...
      return () => {
        ...
        for (const off of localeOffs) off()
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        return installChrome(translate, faces, ctx)
      }, 'dsh-ops-cron: chrome')
    }
