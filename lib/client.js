...
.dsh-ct-treeLead{width:16px;height:20px;flex:none;justify-content:center;align-items:center;display:inline-flex;color:var(--dsw-alias-label-tertiary);font-size:11px;line-height:20px;letter-spacing:0.5px;opacity:.72;user-select:none}
.dsh-ct-depth1{padding-left:20px!important}
.dsh-ct-depth2{padding-left:36px!important}
.dsh-ct-ownerChild{margin-left:0}
...
`
    ...
    function JobList({ t, jobs, runs, selection, expanded, viewer, error, onSelectJob, onSelectRun, onNew, onRun, onToggle, onRemove, onToggleGroup }) {
      ...
      function treeLead() {
        return h('span', { className: 'dsh-ct-treeLead', 'aria-hidden': 'true' }, '\u00b7\u00b7\u00b7')
      }

      function renderRunRows(job, depth) {
        ...
        return jobRuns.map((run) => {
          ...
          return h('div', {
            key: run.id,
            className: joinClass(skin.session, depth >= 2 ? 'dsh-ct-depth2' : 'dsh-ct-depth1'),
            role: 'treeitem',
      ...
      // depth 0: top-level job (normal users). depth 1: job under a user folder (super/fallback).
      function renderJobGroup(job, depth = 0) {
        ...
        return h('div', { className: joinClass(skin.group, nested && 'dsh-ct-ownerChild'), key: job.id },
          h('div', {
            className: joinClass(skin.project, nested && 'dsh-ct-depth1'),
            role: 'treeitem',
            'aria-expanded': open,
      ...
      function renderGrouped() {
        ...
        return ordered.map((group) => {
          ...
          return h('div', { className: skin.group, key: folderId },
            ...
            h('div', { className: 'dsh-ct-runs', 'data-open': open ? 'true' : 'false' },
              h('div', { className: 'dsh-ct-runsInner' },
                group.jobs.map((job) => renderJobGroup(job, 1)),
              ),
            ),
          )
        })
