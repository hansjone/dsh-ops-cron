...
export { callerWorkingDirectory, registerCronTools, resolveCreateCwd, scheduleFromArgs } from './tools.js'
...
function loadPkg(id) {
  ...
  throw new Error(`[dsh-cron-tasks] cannot resolve ${id}\n${errors.join('\n')}`)
}
...
export function apply(ctx, config = {}) {
  ...
  ctx.inject(['settings'], (sctx) => {
    ...
    sctx.logger?.info?.(`[dsh-cron-tasks] settings namespace "${NS}" registered`)
  })

  ctx.inject(['tools'], (tctx) => {
    registerCronTools(tctx, service)
    tctx.logger?.info?.('[dsh-cron-tasks] tools cron_create/list/pause/resume/delete registered')
  })
  ...
  ctx.effect(() => {
    ...
    service.recover()
      ...
      .catch((error) => {
        ctx.logger?.warn?.(`[dsh-cron-tasks] recover failed: ${error instanceof Error ? error.message : error}`)
      })
    return () => {
      stopped = true
      service.stopTimer()
    }
  }, 'dsh-cron-tasks: scheduler')

  ctx.logger?.info?.('[dsh-cron-tasks] host ready')
}
