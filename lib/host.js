...
export const PLUGIN_NAME = 'dsh-cron-tasks'
export const API_PREFIX = '/dsh-cron-tasks'
...
export function defaultCwd() {
  return join(dshHome(), 'cron-tasks', 'workspace')
}
...
export function createHostService(options = {}) {
  ...
  async function recover() {
    await mkdir(join(dshHome(), 'cron-tasks'), { recursive: true })
    const t = now()
    await withState((current) => {
...
export function makeLiveSessionPort(ctx) {
  ...
  return {
    async createAndPrompt({ job, run, text, source }) {
      ...
      const selection = await resolveDefaultModel(ctx)
