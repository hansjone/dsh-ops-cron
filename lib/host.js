...
export function createHostService(options = {}) {
  ...
  /**
   * Re-arm a consumed one-shot job.
   * afterMinutes omitted/0 → immediate run-now; >0 → reschedule next and wait for tick.
   * Auto-enables paused jobs. Rejects cron jobs and in-flight/pending-next oneshots.
   */
  async function retriggerJob(jobId, opts = {}, identity = null) {
    ...
    await withState((current) => {
      ...
      if (job.schedule?.kind !== 'at') {
        const error = new Error('cron_retrigger is only for one-shot (at) jobs; use pause/resume for recurring jobs')
        error.code = 'INVALID_RETRIGGER'
        throw error
      }
