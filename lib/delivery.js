...
export function deliveryDraftFromPeer(peer) {
  ...
  const name = String(peer.label || (isGroup ? t('delivery.wa_group') : t('delivery.wa_dm'))).trim().slice(0, 80)
 ...
 */
export function formatRunResultBody(job, summary, locale = DEFAULT_LOCALE) {
  const name = job?.name || ''
  const text = String(summary || '').trim() || t('delivery.empty_summary', locale, { name })
  const clipped = text.length > IM_MAX_CHARS ? `${text.slice(0, IM_MAX_CHARS)}…` : text
  return `${t('delivery.header', locale, { name })}\n${clipped}`
}
