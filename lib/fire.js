...
export { ACTIVE_RUN_STATUSES } from './fire-status.js'
...
export function publicJob(job, runs = null) {
  if (!job) return null
  const jobRuns = Array.isArray(runs)
    ? runs.filter((run) => run?.jobId === job.id)
    : null
  const projection = jobRuns
    ? projectJobState(job, jobRuns)
    : projectJobState(job, job.lastStatus
      ? [{ status: job.lastStatus, stateEnteredAt: job.lastRunAt, actualAt: job.lastRunAt, exitedAt: job.lastRunAt }]
      : [])
  // When runs not provided, prefer lastStatus-based active hint only for running/queued tip.
  let stateInfo = projection
  if (!jobRuns && (job.lastStatus === 'running' || job.lastStatus === 'queued')) {
    stateInfo = {
      state: job.lastStatus === 'queued' ? 'pending' : 'running',
      stateEnteredAt: job.lastRunAt || job.updatedAt || null,
      activeRunId: null,
      stuck: false,
    }
  } else if (!jobRuns && job.enabled === false) {
    stateInfo = {
      state: 'paused',
  ...
  } else if (!jobRuns && job.schedule?.kind === 'at' && job.nextRunAt == null
    && (job.lastStatus === 'succeeded' || job.lastStatus === 'failed' || job.lastStatus === 'skipped')) {
    stateInfo = {
      state: job.lastStatus === 'succeeded' ? 'succeeded' : 'failed',
      stateEnteredAt: job.lastRunAt || null,
      activeRunId: null,
      stuck: false,
    }
  } else if (!jobRuns) {
  ...
  return {
    ...
    delivery: job.delivery && job.delivery.kind === 'im'
      ? {
        kind: 'im',
        botId: job.delivery.botId,
        targetId: job.delivery.targetId,
        ...job.delivery.mentionJid ? { mentionJid: job.delivery.mentionJid } : {},
        ...job.delivery.mentionName ? { mentionName: job.delivery.mentionName } : {},
      }
      : { kind: 'dsh' },
    mirrorToSession: job.mirrorToSession === true,
    labels: normalizeLabels(job.labels),
    ...job.watch ? { watch: job.watch } : {},
    ...job.progress ? { progress: job.progress } : {},
    ...job.report ? { report: job.report } : {},
    persistHistory: job.persistHistory || { kind: 'retain', limit: 200 },
 ...
 * Also skips when another job with the same role+task labels is already active.
 */
export function claimOccurrence(state, jobId, now, trigger, policies) {
  ...
  if (decision.action === 'fire') {
    const overlap = findLabelOverlapRun(state, job)
    if (overlap) {
      decision = {
        action: 'skip',
        reason: 'label_overlap',
        scheduledAt: decision.scheduledAt ?? now,
  ...
  if (decision.action === 'wait') {
    return { state, job: publicJob(job, state.runs), decision, run: null }
  }

  const run = createRunRecord(job, decision, now, trigger)
  if (decision.action === 'skip') {
    run.status = 'skipped'
    run.stateEnteredAt = now
    run.exitedAt = now
    ...
    } else if (decision.reason === 'label_overlap') {
      run.summary = 'Skipped because another job with the same role+task labels is already running'
    ...
    return { state: next, job: publicJob(nextJob, next.runs), decision, run: alreadySkipped ? null : run }
  }
  ...
  return { state: next, job: publicJob(nextJob, next.runs), decision, run }
}
...
export function settleRun(state, runId, terminal, now) {
  ...
  const status = terminal?.status && !ACTIVE_RUN_STATUSES.has(terminal.status)
    ? terminal.status
    : 'succeeded'
  // Keep stateEnteredAt / actualAt from queued→running so cron_runs duration is real.
  // Only stamp exitedAt (and lastRunAt on the job) at settle time.
  let next = patchRun(state, runId, {
    status,
    summary: terminal?.summary || '',
    error: terminal?.error || null,
    actualAt: run.actualAt || now,
    stateEnteredAt: run.stateEnteredAt || run.actualAt || now,
    exitedAt: now,
    sessionId: run.sessionId,
    ...terminal?.outputRef ? { outputRef: terminal.outputRef } : {},
  })
  if (job) {
    next = upsertJob(next, {
      ...job,
...
export async function executeClaimedRun(state, runId, deps) {
  ...
  if (typeof deps.createAndPrompt !== 'function') {
    ...
    return {
      state: next,
      run: (next.runs || []).find((row) => row.id === runId),
      job: publicJob(getJob(next, job.id), next.runs),
    }
  }
