...
import { deliveryLine, resolveCreateDelivery } from './delivery.js'
...
export function cronToolDefinitions(service, deps = {}) {
  ...
  return [
    {
      name: 'cron_create',
      description: 'Create a DSH 运维定时 job (sidebar scheduled job). NEVER use crontab. NOT schedule_create. Default timezone Asia/Shanghai. For "in N minutes / 一分钟后" pass only after_minutes. For "today/tonight at HH:MM" pass only hour+minute (24h; hour=0 is midnight). Do not also send at or expr. Extra schedule fields are ignored (after_minutes > hour > at > expr). cwd MUST be the current workspace path when chatting in a workspace; if omitted, uses the current session working directory. Pass provider+model for the job; if omitted, snapshots the current session model. Delivery: omit for auto (WhatsApp/IM session → im if a matching 投递目标 exists; else dsh). Or pass delivery=dsh|im with im_bot_id+im_target_id from IM 投递设置.',
      parameters: {
        type: 'object',
        additionalProperties: false,
        properties: {
          name: { type: 'string', description: 'Short display name in the 运维定时 list.' },
      ...
      presentCall: (args) => ({ card: 'generic', title: '创建运维定时', content: String(args?.name || '') }),
    ...
    {
      name: 'cron_list',
      description: 'List DSH 运维定时 jobs (sidebar scheduled jobs). Use this whenever the user asks what scheduled tasks exist. NEVER use crontab -l or /etc/cron* — those are OS crontabs, not this plugin.',
      parameters: {
        type: 'object',
        additionalProperties: false,
        properties: {
      ...
      presentCall: () => ({ card: 'generic', title: '列出运维定时' }),
      async execute(args, exec) {
        aborted(exec)
    ...
    {
      name: 'cron_pause',
      description: 'Pause a scheduled task by id from cron_list / cron_create. It stays in 运维定时 but will not fire until resumed.',
      parameters: {
        type: 'object',
        additionalProperties: false,
      ...
      presentCall: (args) => ({ card: 'generic', title: '暂停运维定时', content: String(args?.id || '') }),
      async execute(args, exec) {
        aborted(exec)
    ...
    },
    {
      ...
      presentCall: (args) => ({ card: 'generic', title: '恢复运维定时', content: String(args?.id || '') }),
      async execute(args, exec) {
        aborted(exec)
    ...
    },
    {
      ...
      presentCall: (args) => ({ card: 'generic', title: '删除运维定时', content: String(args?.id || '') }),
      async execute(args, exec) {
        aborted(exec)
...
export function cronGuidanceText(nowMs = Date.now(), timeZone = 'Asia/Shanghai') {
  ...
  return [
    'This deployment has a DSH sidebar feature named 运维定时 (ops scheduled tasks / dsh-ops-cron).',
    ...
    'Delivery: when chatting on WhatsApp/IM, omit delivery so the job defaults to im (requires a matching 投递目标 for this peer). On Web/DSH, default is dsh. Or pass delivery=im with im_bot_id+im_target_id from IM 投递设置 → 复制调用参数.',
    'When the user asks to look at, create, pause, resume, or delete 运维定时 / 定时任务 / scheduled tasks / cron jobs:',
    '1. If cron_list / cron_create / cron_pause / cron_resume / cron_delete are in your tool list, call them.',
    '2. If they are not listed, search/unlock tools or skills with query "运维定时" or "cron" (skill_search, skill_load, or dev_tool_search) and then call them.',
...
export function makeCronSkill() {
  return {
    ...
    description: '运维定时: 查看、创建、暂停、恢复、删除 DSH 侧栏定时任务（今天晚上几点、一次性执行、scheduled job、cron）。WhatsApp 里创建默认回投 IM；Web 里创建进侧栏会话。不要用系统 crontab，也不是会话内 reminder。',
    whenToUse: 'User asks to list or create 运维定时 / 定时任务, schedule something for tonight/today, pause a job, or mentions cron / scheduled tasks in DeepSeek Harness.',
    source: 'runtime',
    provider: 'runtime',
...
Tools:
- cron_list — list jobs
- cron_create — "一分钟后" → after_minutes=1. Clock time → hour+minute only. Do not send at/expr at the same time. Pass cwd as the current workspace path when creating from a workspace chat. Pass provider+model or inherit the current session model. Delivery auto from session, or delivery=im + im_bot_id + im_target_id.
