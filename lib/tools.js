...
export function cronToolDefinitions(service, deps = {}) {
  ...
  return [
    ...
    {
      name: 'cron_retrigger',
      description: 'Start an extra run now: for recurring (cron) jobs fires immediately without changing the schedule; for consumed one-shots (next=n/a) re-arms/fires a new run. Auto-enables if paused. after_minutes>0 only for one-shots. Rejects if already pending/running. Listeners: when retriggerable=true and progress done_flag is false, call this instead of creating a duplicate job.',
      parameters: {
        type: 'object',
        additionalProperties: false,
        properties: {
...
export function cronGuidanceText(nowMs = Date.now(), timeZone = 'Asia/Shanghai') {
  ...
  return [
    ...
    'One-shot wake: cron_resume only unpauses. For a finished one-shot (retriggerable=true, next=n/a), call cron_retrigger. For a recurring job, cron_retrigger also starts one extra run immediately without changing the cron schedule.',
...
- cron_pause / cron_resume / cron_retrigger / cron_delete — by id from cron_list. Use cron_retrigger to re-fire a consumed one-shot or to immediately start one extra run of a recurring job.
`,
  }
}
