...
export function cronToolDefinitions(service, deps = {}) {
  ...
  return [
    ...
    {
      name: 'cron_retrigger',
      description: 'Re-fire a consumed one-shot job (succeeded/failed/skipped with next=n/a) as a new run. Auto-enables if paused. Use after_minutes>0 to delay. For recurring jobs use pause/resume. Listeners: when retriggerable=true and progress done_flag is false, call this instead of creating a new job.',
      parameters: {
        type: 'object',
        additionalProperties: false,
        properties: {
          task_id: { type: 'string', description: 'One-shot job id (alias: id).' },
          id: { type: 'string', description: 'Alias of task_id.' },
          after_minutes: { type: 'number', description: 'Delay before fire. Omit or 0 = immediate run-now.' },
        },
      },
      output: {
        schema: {
...
export function cronGuidanceText(nowMs = Date.now(), timeZone = 'Asia/Shanghai') {
  ...
  return [
    ...
    'One-shot wake: cron_resume only unpauses. For a finished one-shot (retriggerable=true, next=n/a), call cron_retrigger to start a new run on the same job id — do not create a duplicate executor.',
    'When the user asks to look at, create, pause, resume, retrigger, or delete 定时任务 / scheduled tasks / cron jobs:',
    '1. If cron_list / cron_create / cron_pause / cron_resume / cron_retrigger / cron_delete / cron_query / cron_runs / cron_progress are in your tool list, call them.',
...
- cron_pause / cron_resume / cron_retrigger / cron_delete — by id from cron_list. Use cron_retrigger (not resume) to re-fire a consumed one-shot worker.
`,
  }
}
