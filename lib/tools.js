...
function jobLine(job) {
  ...
  return `${job.name} [${state}] ${sched} tz=${tz} cwd=${job.cwd || '(recent workspace)'} next=${when} id=${job.id}`
}
  ...
  return [
    {
      ...
      output: {
        ...
        render: (_args, value) => {
          ...
          return text(`Created "${job?.name}" (${job?.id}) ${sched} in ${tz}.${cwd}. Next run: ${when}. If that local time is wrong, delete this job and recreate with 24h hours (22=10pm).`)
        },
      },
...
Tools:
- cron_list — list jobs
- cron_create — "一分钟后" → after_minutes=1. Clock time → hour+minute only. Do not send at/expr at the same time. Pass cwd as the current workspace path when creating from a workspace chat.
